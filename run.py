...
def intro():
    ...
    print("\nWelcome to\n")
    print(intro_art)
    print("\nA game in which you guess the letters")
    print("in a word in order to stay alive!!")

    # Validate the inputs for this function - with custom user prompts
...
def help():
    ...
    print("\nThe objective of the game is to guess the"
          "word before you run out of lives.")
    print("You can only guess a letter at a time.")
    print("If you guess a letter that is in the word,"
          "it will be revealed.")
    print("If you guess a letter that is not in the word,"
...
def display_dashes(word, correct_guesses):
    '''
    Display dashes corresponding to the length of the word with
    ...
    Returns:
        str: A string with dashes and correctly guessed letters,
        with spaces between each character.
    '''

    return ' '.join([letter if letter in
                    correct_guesses else '-' for letter in word])
...
def play_game(level):
    ...
    while True:
        ...
        if choice == "P":
            new_random_word, new_definition = get_nrw_nd(level)
            play_game(level)
            break
        elif choice == "L":
            level_select()
            ...
            print("\nInvalid input.\nEnter 'P' - play again\n"
                  "Enter 'L' - return to level select\n"
                  "Enter 'E' - exit the game")
